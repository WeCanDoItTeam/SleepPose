...
def get_sleep_day(user_id,st_dt,ed_dt):
    ...
    try:
        st_dt = st_dt - timedelta(minutes=1)
        ed_dt = ed_dt + timedelta(minutes=1)
...
def calculate_report_stats(data, class_map):
    ...
    for row in data:
        ...
        if st_dt and ed_dt and class_id in stats:
...
def report_window():
    with st.spinner(f"📊 수면 분석 리포트를 출력하는 중..."):
        time.sleep(3)
        ...
        # 2. 데이터 로드
        pose_data, audio_data = get_sleep_day(user_id, start_time, end_time)
